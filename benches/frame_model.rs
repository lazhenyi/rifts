...
use rifts::frame::{Codec, Frame, FrameFlags, FrameType, Priority};
...
fn bench_codec_tag(c: &mut Criterion) {
    ...
    group.bench_function("json_tag", |b| {
        b.iter(|| black_box(Codec::Json.tag()));
    });
    group.bench_function("cbor_tag", |b| {
        b.iter(|| black_box(Codec::Cbor.tag()));
    });
    group.bench_function("from_tag", |b| {
        b.iter(|| black_box(Codec::from_tag(black_box(b'B'))));
    });
