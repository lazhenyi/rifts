...
use rifts::codec::{CborCodec, Codec, CodecExt, JsonCodec, negotiate};
use rifts::frame::Codec as FrameCodec;
...
fn bench_negotiate(c: &mut Criterion) {
    let server: Vec<Arc<dyn Codec>> = vec![Arc::new(CborCodec), Arc::new(JsonCodec)];
    let mut group = c.benchmark_group("codec/negotiate");
    group.bench_function("match_first", |b| {
        let client = vec![FrameCodec::Cbor, FrameCodec::Json];
        b.iter(|| negotiate(black_box(&server), black_box(&client)).expect("neg"));
    });
    group.bench_function("match_second", |b| {
        let client = vec![FrameCodec::Json, FrameCodec::Cbor];
        b.iter(|| negotiate(black_box(&server), black_box(&client)).expect("neg"));
    });
    group.bench_function("no_match", |b| {
        let client = vec![FrameCodec::Json];
        let server_json: Vec<Arc<dyn Codec>> = vec![Arc::new(JsonCodec)];
